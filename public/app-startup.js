(function empyreanStartupSync() {
    ...
    var _fbRetryTimer = setInterval(function() {
        ...
        if (window._firebaseLoaded) {
            ...
            // Kick off real-time listeners if they haven't started yet
            if (typeof window._startRealtimeListeners === 'function' && !window._postsListener) {
                console.log('[Empyrean] Starting real-time listeners after Firebase late-init');
        ...
        if (typeof firebase !== 'undefined' && typeof window._initFirebase === 'function') {
            ...
            if (ok) {
                clearInterval(_fbRetryTimer);
                console.log('[Empyrean] ✅ Firebase initialized on retry ' + _fbRetry);
                // Kick off listeners now that Firebase is ready
                setTimeout(function() {
                    if (typeof window._startRealtimeListeners === 'function' && !window._postsListener) {
                        window._startRealtimeListeners();
                    }
        ...
        if (_fbRetry >= 20) {
            clearInterval(_fbRetryTimer);
            console.warn('[Empyrean] Firebase did not load after 20 retries — running offline mode.');
        }
    }, 500);
