document.addEventListener('DOMContentLoaded', function() {
                ...
                if (!guestMode) {
                    ...
                    window._startRealtimeListeners = function() {
                        ...
                        if (!window._postsListener) {
                            window._postsListener = db.collection('posts').limit(40)
                        ...
                        if (!window._newsListener) {
                            window._newsListener = db.collection('news_posts').limit(20)
                                .onSnapshot(function(snap) {
                                    if (!snap) return;
                        ...
                        if (!window._mktListener) {
                            window._mktListener = db.collection('marketplace_listings').limit(40)
                                .onSnapshot(function(snap) {
                                    if (!snap) return;
                        ...
                        if (!window._reelsListener) {
                            window._reelsListener = db.collection('reels').limit(30)
                                .onSnapshot(function(snap) {
                                    if (!snap) return;
                                    snap.docChanges().forEach(function(change) {
            ...
            function navigateTo(targetId, fromClick = false) {
                ...
                if (window._navigating) {
                    console.warn('[Nav] Navigation already in progress — queuing:', targetId);
                    setTimeout(function() { navigateTo(targetId, fromClick); }, 300);
                    return;
                }
                window._navigating = true;
                window._navigatingTo = targetId;
                // Safety: always unlock navigation within 3s max
                const _navGuard = setTimeout(function() { window._navigating = false; window._navigatingTo = null; }, 3000);
                try {
                    ...
                    switch (form.id) {
                            ...
                            (async () => {
                                try {
                                    ...
                                    try {
                                        ...
                                        setTimeout(async () => { try { await fbDb.collection('posts').doc(newPost.dataset.postId).set({ id: newPost.dataset.postId, userId: userState.id, username: userState.fullName || userState.username, text, media: cloudUrls.filter(u=>u), createdAt: new Date().toISOString() }); } catch(e2) {} }, 3000);
                                    }
                                    // Clear quote state
                        ...
                        case 'marketplace-form': {
                            ...
                            (async () => {
                                try {
                                    ...
                                    if (window.fbDb) {
                                        // Notify community of new marketplace item
                                        ...
                                        window.fbDb.collection('marketplace_listings').doc(newId).set({
                                            id: newId, name: itemName, price: itemPrice,
                                            currency: selectedCurrency, salesType,
                                            location: itemLocationInput?.value || '',
                                            sellerId: userState.id,
                                            sellerName: userState.username,
                                            media: cloudUrls,
                                            createdAt: new Date().toISOString()
                                        }).catch(e => console.warn('Mkt save failed:', e.message));
                                    }
                                    ...
                                    try {
                                        await fbDb.collection('marketplace_listings').doc(newId).set({
                                            ...
                                            media: cloudUrls, createdAt: new Date().toISOString()
                                        });
                                    } catch(e) { console.warn('Marketplace Firestore save failed:', e.message); }
                             ...
                             (async () => {
                                 try {
                                     ...
                                     window.uploadToCloudinary(videoFile, (pct) => { if (pct % 25 === 0) showNotification(`Reel upload: ${pct}%`, 'info'); })
                                         .then(async (cloudUrl) => {
                                             // Update the card's video src with permanent cloud URL
                                             ...
                                             try {
                                                 await fbDb.collection('reels').doc(newId).set({
                                                     ...
                                                     videoUrl: (cloudUrl && !cloudUrl.startsWith('blob:')) ? cloudUrl : null,
                                                     createdAt: new Date().toISOString()
                                                 });
                                             } catch(e) {}
                        ...
                        <span class="action-btn view-count-display" style="margin-left:auto;color:var(--text-muted);font-size:0.75rem;pointer-events:none;display:flex;align-items:center;gap:4px;"><i class="fas fa-eye"></i><span class="view-count">0</span></span>
                                    ...
                                    await fbDb.collection('news_posts').doc(newsId).set({
                                            id: newsId, title, content, mediaUrl, mediaType,
